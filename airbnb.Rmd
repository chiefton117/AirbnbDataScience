...
makeQuartile <- function(data) {
  ...
  #   include.lowest=TRUE))
    cut(data,
    breaks=quantile(data, probs=seq(0,1, by=0.25), na.rm=TRUE),
    include.lowest=TRUE)
}
...
dt_frame$quartile <- makeQuartile(dt_frame$priceNum)

summary(dt_frame$priceNum)
summary(dt_frame$quartile)
