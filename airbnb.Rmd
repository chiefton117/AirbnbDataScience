...
stdev <- sd(sData$priceNum)
mean <- mean(sData$priceNum)
...
clrs <- colorBin(palette = "RdYlBu", domain = sData$priceNum, reverse = TRUE, bins = 11)
...
leaflet(sData) %>%
  ...
  addCircleMarkers(data = sData,
           ...
           lat = ~latitude,
           color = ~clrs(priceNum),
           stroke = FALSE, fillOpacity = 0.4, radius = 5) %>%
