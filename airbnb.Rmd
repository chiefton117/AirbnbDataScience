...
#dt_frame <- lapply(dt_frame[, sapply(dt_frame, is.character)], factorize)

idx <- 1:ncol(dt_frame)
dt_frame[ , idx] <- lapply(dt_frame[ , idx], as.factor)
#dt_frame[sapply(dt_frame, is.character)] <- lapply(dt_frame[sapply(dt_frame, is.character)], as.factor)

#dt_frame <- as.data.frame(unclass(dt_frame))
...
bnb_train_lbl <- dt_frame_lbl[train_sample,1]
...
bnb_model <- C5.0(x = bnb_train, bnb_train_lbl)
summary(bnb_model)
