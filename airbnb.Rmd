...
# Make quartiles out of numerical data for easier interpretation??
makeQuartile <- function(data) {
  # with(dt_frame, cut(priceNum, 
  #   breaks=quantile(priceNum, probs=seq(0,1, by=0.25), na.rm=TRUE), 
  #   include.lowest=TRUE))
    cut(data,
    breaks=unique(quantile(data, probs=seq(0,1, by=0.25), na.rm=TRUE)),
    include.lowest=TRUE,
    labels = FALSE)
}

# Classify a review score as good, decent, or bad
...
#dt_frame <- listings %>% select("review_scores_rating", "host_response_time", "host_response_rate", "host_acceptance_rate", "host_is_superhost", "host_listings_count", "host_total_listings_count", "host_has_profile_pic", "host_identity_verified", "neighbourhood_cleansed", "neighbourhood_group_cleansed", "property_type", "room_type", "accommodates", "bedrooms", "beds", "minimum_nights", "maximum_nights", "has_availability", "availability_365", "number_of_reviews", "number_of_reviews_ltm", "priceNum") %>% filter(!is.na(review_scores_rating))

dt_frame <- listings %>% select("review_scores_rating", "host_response_time", "host_response_rate", "host_acceptance_rate", "host_is_superhost", "host_listings_count", "host_has_profile_pic", "host_identity_verified", "neighbourhood_group_cleansed", "property_type", "room_type", "accommodates", "bedrooms", "beds", "minimum_nights", "maximum_nights", "has_availability", "availability_365", "number_of_reviews", "number_of_reviews_ltm", "priceNum") %>% filter(!is.na(review_scores_rating))


# Create classification variable - This works, but only creates two classes
#dt_frame <- cbind(Rating = as.factor(ifelse(dt_frame$review_scores_rating >= 90, "good", "poor")), dt_frame)

# Experimental third classification
dt_frame <- cbind(Rating = as.factor(function() {
  d = dt_frame$review_scores_rating
    if(d >= 90) {
      return(as.factor("good"))
    } else if(d < 90 & d >= 80) {
      return(as.factor("decent"))
    } else {
      return(as.factor("bad"))
    }
}), dt_frame)

# Causing """ Error in order(y) : unimplemented type 'list' in 'orderVector1' """ error
#dt_frame <- cbind(Rating = as.factor(lapply(dt_frame$review_scores_rating, classify)), dt_frame)

# Another possible approach?
#dt_frame$review_scores_rating <- lapply(dt_frame$review_scores_rating, classify)

# Take out review scores variable
dt_frame <- dt_frame[-2]

# We'll convert some values to quartiles for easier readibility
to_quartiles <- c("host_response_rate", "host_acceptance_rate", "host_listings_count", "minimum_nights", "maximum_nights", "availability_365", "number_of_reviews_ltm", "accommodates")

dt_frame$host_response_rate <- as.numeric(sub("%","", dt_frame$host_response_rate))/100
dt_frame$host_acceptance_rate <- as.numeric(sub("%","", dt_frame$host_acceptance_rate))/100

dt_frame[, colnames(dt_frame) %in% to_quartiles] <- lapply(dt_frame[, colnames(dt_frame) %in% to_quartiles], makeQuartile)




dt_frame$priceNum <- makeQuartile(dt_frame$priceNum)
#dt_frame$availability_365 <- makeQuartile(dt_frame$availability_365)
dt_frame$number_of_reviews <- makeQuartile(dt_frame$number_of_reviews)
#dt_frame$number_of_reviews_ltm <- makeQuartile(dt_frame$number_of_reviews_ltm)


#dt_frame <- listings %>% select("review_scores_rating", "review_scores_rating", "property_type", "room_type", "bedrooms", "number_of_reviews_ltm", "host_response_time") %>% filter(!is.na(review_scores_rating))


#, "minimum_nights", "maximum_nights", "host_is_superhost", "host_listings_count", "host_total_listings_count", "neighbourhood_group_cleansed", "accommodates", "host_has_profile_pic", "host_identity_verified"

#, "neighbourhood_cleansed" # YOU!!!!!

# Removed for less data cleaning work
#"amenities", "host_verifications"
#"latitude", "longitude"


# Code to convert all cols to factors
idx <- 2:ncol(dt_frame)
dt_frame[ , idx] <- lapply(dt_frame[ , idx], as.factor)

# Convert all characters to factors
#[sapply(dt_frame, is.character)] <- lapply(dt_frame[sapply(dt_frame, is.character)], as.factor)
...
bnb_train <- dt_frame[train_sample,-1]
bnb_train_lbl <- dt_frame[train_sample,1]

# Create test set and labels from any values not in training set
bnb_test <- dt_frame[-train_sample,-1]
bnb_test_lbl <- dt_frame[-train_sample,1]
...
rmNullLevels <- function(df) {
  for(i in 1:ncol(df)) {
    if(is.factor(df[, i])) {
      lvls <- levels(df[, i])
      if(!is.null(lvls) && lvls[1] == "") {
        levels(df[,i])[1] = "N/A"
      }
    }
...
bnb_train <- rmNullLevels(bnb_train)

summary(bnb_train)


bnb_model <- C5.0(x = bnb_train, y= bnb_train_lbl)
...
scores <- scores %>% drop_na() %>% filter(price <= 1000) # %>% filter(Score >= 80)
