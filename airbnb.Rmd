...
stopWords <- c("a", "about", "above", "above", "across", "after", "afterwards", "again", "against", "all", "almost", 
    ...
    "can", "cannot", "cant", "co", "con", "could", "couldnt", "de", "describe", "detail", "did", "do", "done", "don't", 
...
words <- setdiff(str_split(sample, " ", simplify=TRUE), stopWords)
words <- data.frame(setdiff(words, letters))
colnames(words) <- c("word")
#remove all 3 letter or less words
words[nchar(words$word) > 3,]

tibble(words)

ggplot(data=words, mapping=aes(label=word)) +
  geom_text_wordcloud() +
  theme_bw()
