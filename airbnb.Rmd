...
stopWords <- c("a", "about", "above", "above", "across", "after", "afterwards", "again", "against", "all", "almost", 
    ...
    "can", "cannot", "cant", "co", "con", "could", "couldnt", "de", "describe", "detail", "do", "done", "don't", 
    ...
    "that", "the", "their", "them", "themselves", "the", "then", "thence", "there", "thereafter", "thereby", "therefore", 
...
reviews$comments[3]<-gsub("[\r\n\\.\\,]","",reviews$comments[3])

reviews$comments[3] <- tolower(gsub('[[:punct:] ]+',' ',reviews$comments[3]))

#reviews$comments[3]

#a <- as.data.frame(str_split(reviews$comments[3], " "))
a <- str_split(reviews$comments[3], " ", simplify=TRUE)

# %>% a - intersection(a, stopWords)

#a.freq<-table(unlist(a))
#cbind(names(a.freq),as.integer(a.freq))
b <- setdiff(a, stopWords)
b <- setdiff(b, letters)
b

#b <- a[!(a %in% stopWords)]
#b = subset(a, !(a %in% stopWords))
#b <- a %>% filter(a %in% stopWords)
